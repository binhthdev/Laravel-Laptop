    ...
    if(Auth::check()){
        ...
        if($cart){
            ...
            if($product){
                ...
                if($quantity_diff > 0 && $quantity_diff > $product->quantity){
                    return response()->json([
                        'success' => false,
                        'message' => 'Số lượng yêu cầu vượt quá số lượng có sẵn. Chúng tôi chỉ còn ' . $product->quantity . ' sản phẩm này trong kho.'
                    ], 400);
                }
                ...
                $unit_price = $cart->price / $cart->quantity;
                $cart->quantity = $quantity;
                $cart->price = $unit_price * $quantity;
                $cart->save();

                return response()->json([
                    'success' => true,
                    'message' => 'Giỏ hàng đã được cập nhật!',
                    'new_total_price' => number_format($cart->price, 2),
                ]);
            }
        }
        return response()->json([
